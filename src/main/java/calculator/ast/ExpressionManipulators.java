...
public class ExpressionManipulators {
    ...
    private static double toDoubleHelper(IDictionary<String, AstNode> variables, AstNode node) {
        ...
        if (node.isNumber()) {
            ...
            //double check structure of variables dict
            ...
            switch (operation) {
                case "+":
                    result = toDoubleHelper(variables, children.get(0)) + toDoubleHelper(variables, children.get(1));
                    break;
                case "-":
                    result = toDoubleHelper(variables, children.get(0))- toDoubleHelper(variables, children.get(1));
                    break;
                ...
                case "/":
                    ...
                    break;
                case "^":
                    result = Math.pow(toDoubleHelper(variables, children.get(0)),
                            toDoubleHelper(variables, children.get(1)));
                    break;
                case "negate":
                    result = -1 * toDoubleHelper(variables, children.get(0));
                    break;
                case "sin":
                    result = Math.sin(toDoubleHelper(variables, children.get(0)));
                    break;
                case "cos":
                    result = Math.cos(toDoubleHelper(variables, children.get(0)));
                    break;
                default:
                    throw new EvaluationError("Operation is not recognized.");
            }
            return result;
        }
    }
    ...
    public static AstNode handleSimplify(Environment env, AstNode node) {
        ...
        AstNode simplify = node.getChildren().get(0);
        return simplifyHelper(env, simplify);
    }
    ...
    private static AstNode simplifyHelper(Environment env, AstNode node) {
        ...
        if (node.isNumber()) {
            ...
            if (env.getVariables().containsKey(node.getName())) {
                if (env.getVariables().get(node.getName()).isNumber()) {
                    return new AstNode(env.getVariables().get(node.getName()).getNumericValue());
                }else {
                    return simplifyHelper(env, env.getVariables().get(node.getName()));
                }
            } else {
                return node;
            }
            ...
            if (operation.equals("+") || operation.equals("-") || operation.equals("*")) {
                leftResult = simplifyHelper(env, node.getChildren().get(0));
                rightResult = simplifyHelper(env, node.getChildren().get(1));
                children.add(leftResult);
                children.add(rightResult);
                
                if (leftResult.isNumber() && rightResult.isNumber()) {
                    IList<AstNode> child = new DoubleLinkedList<>();
                    child.add(new AstNode(operation, children));
                    return handleToDouble(env, new AstNode("toDouble", child));
                }else {
                    return new AstNode(operation, children);
                }
            }else {
                leftResult = simplifyHelper(env, node.getChildren().get(0));
                children.add(leftResult);
     ...
     */
    private static void checkInput(Environment env, AstNode expression, String variable, 
            double varMin, double varMax, double step) {
        if (varMin > varMax) {
            throw new EvaluationError("varMin can not be more than varMax");
        }
        if (env.getVariables().containsKey(variable)) {
            throw new EvaluationError("var is already defined");
        }
        if (step <= 0) {
            throw new EvaluationError("step can not be zero or negative");
        }
        
        if (undefinedExists(env, expression, variable)) {
            throw new EvaluationError("expression contains an undefined variable");
        }
    }
    
    private static boolean undefinedExists(Environment env, AstNode node, String varName) {
        for (AstNode child : node.getChildren()) {
            if (child.isVariable() && !child.getName().equals(varName) 
                    && !env.getVariables().containsKey(child.getName())) {
                return true;
            }else {
                return undefinedExists(env, child, varName);
            }
        }
        return false;
    }
    ...
    public static AstNode plot(Environment env, AstNode node) {
        ...
        String variable = children.get(1).getName();
        ...
        checkInput(env, expression, variable, varMin, varMax, step);
        
        IList<Double> xValues = new DoubleLinkedList<>();
        IList<Double> yValues = new DoubleLinkedList<>();
        for (double i = varMin; i <= varMax; i += step) {
            env.getVariables().put(variable, new AstNode(i));
            System.out.println(i);
            System.out.println(toDoubleHelper(env.getVariables(), expression));
            xValues.add(i);
